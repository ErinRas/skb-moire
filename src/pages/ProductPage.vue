<template>
  ...
  <main class="content container" v-else-if="isLoadingFailed">
    <h1>
      Произошла ошибка при загрузке товара. Пожалуйста, попробуйте зайти сюда
      позже.
    </h1>
  </main>
  <main class="content container" v-else>
    ...
    <section class="item">
      <div class="item__pics pics">
        ...
        <ul class="pics__list">
          <li class="pics__item" v-for="image in gallery" :key="image.id">
            <a href="" class="pics__link" :class="{
              'pics__link--current': image.id === activeColorId,
        ...
        <div class="item__form">
          <form class="form" action="#" method="POST">
            <div class="item__row item__row--center">
              <base-item-counter v-model="productAmount" :itemAmount="productAmount"
                ...
                <ul class="colors colors--black">
                  <li class="colors__item" v-for="color in product.colors" :key="color.id">
                    <label class="colors__label">
                      <input class="colors__radio sr-only" type="radio" name="color" :value="color.color.id"
                        :checked="activeColorId === color.color.id" v-model="activeColorId" />
            ...
            <p v-if="isSendingFailed">
              Произошла ошибка при добавлении товара в корзину. Попробуйте ещё
              раз!
            </p>
      ...
      <div class="item__desc">
        <ul class="tabs">
          <li class="tabs__item">
            <a class="tabs__link" :class="{
              'tabs__link--current': selectedTab === 'base-item-content',
            }" @click="setSelectedComponent('base-item-content')">
              Информация о товаре
            </a>
          </li>
          <li class="tabs__item">
            <a class="tabs__link" :class="{
              'tabs__link--current': selectedTab === 'base-item-delivery',
            }" @click="setSelectedComponent('base-item-delivery')">
              Доставка и возврат
            </a>
          </li>
        </ul>
