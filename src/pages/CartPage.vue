    ...
    <section class="cart">
      <form class="cart__form form" action="#" method="POST">
        <div class="cart__field">
          <ul class="cart__list">
            <cart-item v-for="item in cart" :key="item.id" :size="item.size.title" :title="item.product.title"
              ...
              :amount="item.quantity"></cart-item>
          </ul>
        </div>
...
export default {
  ...
  computed: {
    ...
    totalPrice() {
      return numberFormat(this.$store.getters.cartTotalPrice)
    },
    totalQuantity() {
