      ...
      <form class="cart__form form" action="#" method="POST" @submit.prevent="sendOrder">
        ...
        <div class="cart__block">
          ...
          <div class="cart__total">
            <p>
              Доставка:
              <b v-if="deliveryPrice">{{ `${deliveryPrice} ₽` }} </b> 
              <b v-else>бесплатно</b>
            </p>
            <p>
              Итого: <b>{{ totalQuantity }}</b> товара на сумму
              <b>{{ totalPrice }} ₽</b>
            </p>
          </div>
...
export default {
  ...
  computed: {
    ...
    totalPrice() {
      return numberFormat(this.$store.getters.cartTotalPrice + this.deliveryPrice)
    },
    totalQuantity() {
    ...
    deliveryPrice() {
      return this.deliveries
        ? +this.deliveries.find(
          (delivery) => delivery.id === this.formData.deliveryId
        ).price
        : 0
    },
    formError() {
